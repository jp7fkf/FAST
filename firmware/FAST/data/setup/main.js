...
function setStation() {
  ...
  $.get('/mode/station', {
     ...
     ssid : ($('#form [name="ssid"]').val() == "stealth-ssid")
                ? $('#form [name="stealth-ssid"]').val()
                : $('#form [name="ssid"]').val(),
    ...
    $.get('/wifi/confirm')
        .done(function(res) {
          ...
          if (res != "false") {
            ...
            timerID = null;
            $('#info-status')
                .html('Connection Successful.<br/>URL: <a href="http://' + res +
                      '/" target="_blank">http://' + res + '/</a>');
          }
        })
        .fail(function() {
          $('#info-status').text('Connection Failed. Please Reload.');
