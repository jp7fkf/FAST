...
function updateStatus(status) {
  ...
  $('#log-area')
      .prepend($('<p>').text(Date().match(/.+(\d\d:\d\d:\d\d).+/)[1] + ': ' +
                             status));
}
function load() {
  $.getJSON('/info', {}, function(data) {
     ...
     $('#input-www_auth_method')
         .val(www_auth_method_map[data["www_auth_method"]]);
...
$('#manage select[name="action"]').change(function() {
  ...
  $('#input-time')
      .val((new Date((new Date()).getTime() + 9 * 60 * 60 * 1000))
               .toISOString()
               .substring(0, 17) +
           "00");
  switch (action) {
...
function manage() {
  ...
  case "change-ip":
    ...
    if (!local_ip.match(/^\d{1,3}(\.\d{1,3}){3}$/))
      return $('#form-submit label').text("Invalid IP address");
    if (!subnetmask.match(/^\d{1,3}(\.\d{1,3}){3}$/))
      return $('#form-submit label').text("Invalid Subnet Mask");
    if (!gateway.match(/^\d{1,3}(\.\d{1,3}){3}$/))
      return $('#form-submit label').text("Invalid Gateway IP");
  ...
  case "settings":
    ...
    if (!www_auth_method.match(/^(digest|basic)$/))
      return $('#form-submit label').text("Invalid Authentication Method");
    // if(!www_username.match(/^\d{1,3}(\.\d{1,3}){3}$/))return $('#form-submit
    // label').text("Invalid Subnet Mask");
    // if(!www_password.match(/^\d{1,3}(\.\d{1,3}){3}$/))return $('#form-submit
    // label').text("Invalid Gateway IP");
    if (!confirm('Are you sure to change settings?'))
      return;
